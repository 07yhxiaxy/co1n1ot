...
char test_file_A[] = "0:FlagA.txt"; //Flag A txt file
char test_file_B[] = "0:FlagB.txt"; //Flag B txt file
char test_bin_A[] = "0:TestA.bin"; //test A bin file
char test_bin_B[] = "0:TestB.bin"; //test B bin file
...
int main(void)
{
	  ...
	  /*3.) STARTS BOOTLOADER HERE!*/
				...
				//read SD card file
				test_file_A[0] = LUN_ID_SD_MMC_0_MEM + '0';
				test_file_B[0] = LUN_ID_SD_MMC_0_MEM + '0';
		
				res = f_open(&file_object, (char const *)test_file_A, FA_READ);
						if (res == FR_OK)
						{
							flagA = true;
						}
						...
						{
							res = f_open(&file_object, (char const *)test_file_B, FA_READ);
							if (res != FR_OK)
							{
								flagA = true;
							}
				...
				if(flagA){
					test_bin_A[0] = LUN_ID_SD_MMC_0_MEM + '0';
					if(!Runbinfile(test_bin_A)){goto fw_update_error;}
					f_unlink(test_file_A);//delete FlagA after creating it.
				}
				else
				{	
					test_bin_B[0] = LUN_ID_SD_MMC_0_MEM + '0';
					if(!Runbinfile(test_bin_B)){goto fw_update_error;}
					f_unlink(test_file_B);//delete FlagB after creating it.
				}
